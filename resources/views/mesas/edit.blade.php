...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-2xl mx-auto bg-surface rounded-lg shadow-lg p-6">
        <h1 class="text-2xl font-bold text-text mb-6">✏️ Editar Mesa {{ $mesa->numero_mesa }}</h1>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="estado" 
                        ...
                        <option value="ocupado" {{ old('estado', $mesa->estado) == 'ocupado' ? 'selected' : '' }}>Ocupado</option>
                        <option value="reservado" {{ old('estado', $mesa->estado) == 'reservado' ? 'selected' : '' }}>Reservado</option>
                        <option value="fuera_servicio" {{ old('estado', $mesa->estado) == 'fuera_servicio' ? 'selected' : '' }}>Fuera de Servicio</option>
                    </select>
                </div>

                <div id="reservaFields" style="{{ old('estado', $mesa->estado) == 'reservado' ? 'display: block;' : 'display: none;' }}">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('cliente_reserva', $mesa->cliente_reserva) }}" 
                            ...
                            <div>
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('telefono_reserva', $mesa->telefono_reserva) }}" 
                                       class="w-full px-4 py-2 border border-border rounded-lg focus:outline-none focus:border-primary">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" 
                                       name="hora_reserva" 
                                       value="{{ old('hora_reserva', $mesa->hora_reserva ? \Carbon\Carbon::parse($mesa->hora_reserva)->format('Y-m-d\TH:i') : '') }}" 
                                       class="w-full px-4 py-2 border border-border rounded-lg focus:outline-none focus:border-primary">
                                @error('hora_reserva')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
